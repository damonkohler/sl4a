...
public class SpeechRecognitionFacade implements RpcReceiver {
  ...
  public String recognizeSpeech(
      @RpcOptionalString(description = "prompt: text prompt to show to the user when asking them to speak") final String prompt,
      @RpcOptionalString(description = "language: language override to inform the recognizer that it should expect speech in a language different than the one set in the java.util.Locale.getDefault()") final String language,
      @RpcOptionalString(description = "languageModel: Informs the recognizer which speech model to prefer (see android.speech.RecognizeIntent)") final String languageModel) {
