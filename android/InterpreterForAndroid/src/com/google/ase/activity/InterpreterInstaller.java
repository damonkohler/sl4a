...
public abstract class InterpreterInstaller extends Activity {
  ...
  protected void downloadInterpreter() {
    Intent intent = new Intent(this, UrlDownloader.class);
    intent.putExtra(InterpreterConstants.EXTRA_URL, mDescriptor.getInterpreterArchiveUrl());
    intent.putExtra(InterpreterConstants.EXTRA_OUTPUT_PATH, InterpreterConstants.DOWNLOAD_ROOT);
    startActivityForResult(intent, RequestCode.DOWNLOAD_INTERPRETER.ordinal());
  }

  protected void downloadInterpreterExtras() {
    Intent intent = new Intent(this, UrlDownloader.class);
    intent.putExtra(InterpreterConstants.EXTRA_URL, mDescriptor.getExtrasArchiveUrl());
    intent.putExtra(InterpreterConstants.EXTRA_OUTPUT_PATH, InterpreterConstants.DOWNLOAD_ROOT);
    startActivityForResult(intent, RequestCode.DOWNLOAD_INTERPRETER_EXTRAS.ordinal());
  }

  protected void downloadScripts() {
    Intent intent = new Intent(this, UrlDownloader.class);
    intent.putExtra(InterpreterConstants.EXTRA_URL, mDescriptor.getScriptsArchiveUrl());
    intent.putExtra(InterpreterConstants.EXTRA_OUTPUT_PATH, InterpreterConstants.DOWNLOAD_ROOT);
    startActivityForResult(intent, RequestCode.DOWNLOAD_SCRIPTS.ordinal());
  }

  protected void extractInterpreter() {
    Intent intent = new Intent(this, ZipExtractor.class);
    intent.putExtra(InterpreterConstants.EXTRA_INPUT_PATH, new File(
        InterpreterConstants.DOWNLOAD_ROOT,
        mDescriptor
        .getInterpreterArchiveName()).getAbsolutePath());
    intent.putExtra(InterpreterConstants.EXTRA_OUTPUT_PATH, InterpreterUtils.getInterpreterRoot(
        this)
        .getAbsolutePath());
    startActivityForResult(intent, RequestCode.EXTRACT_INTERPRETER.ordinal());
  }

  protected void extractInterpreterExtras() {
    Intent intent = new Intent(this, ZipExtractor.class);
    intent.putExtra(InterpreterConstants.EXTRA_INPUT_PATH, new File(
        InterpreterConstants.DOWNLOAD_ROOT,
        mDescriptor
        .getExtrasArchiveName()).getAbsolutePath());
    intent.putExtra(InterpreterConstants.EXTRA_OUTPUT_PATH,
        InterpreterConstants.INTERPRETER_EXTRAS_ROOT);
    startActivityForResult(intent, RequestCode.EXTRACT_INTERPRETER_EXTRAS.ordinal());
  }

  protected void extractScripts() {
    Intent intent = new Intent(this, ZipExtractor.class);
    intent.putExtra(InterpreterConstants.EXTRA_INPUT_PATH, new File(
        InterpreterConstants.DOWNLOAD_ROOT,
        mDescriptor
        .getScriptsArchiveName()).getAbsolutePath());
    intent.putExtra(InterpreterConstants.EXTRA_OUTPUT_PATH, InterpreterConstants.SCRIPTS_ROOT);
